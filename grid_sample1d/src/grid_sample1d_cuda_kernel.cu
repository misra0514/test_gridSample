...
torch::Tensor grid_sample1d_cuda_forward(
  ...
  // TODO: 
    ...
    const auto C = 1;
    const auto L_in = 1024*1024 *256 *128 ;
    void* tempInput;
    uint64_t size = L_in*sizeof(input.type());
    // cudaMalloc((void**) &tempInput, size);
    cudaMallocManaged((void**) &tempInput, size);
    cudaMemset(tempInput , 10, size);

  torch::Tensor output = torch::ones({batch_size, C, L_out}, input.options());
...
//  const dim3 blocks((C*L_out + threads - 1) / threads, batch_size);
  ...
  // AT_DISPATCH_FLOATING_TYPES(input.type(), "lltm_forward_cuda", ([&] {
    grid_sample1d_cuda_forward_kernel<float><<<blocks, threads>>>(
        // input.data<scalar_t>(),
        (float*)tempInput,
        grid.data<float>(),
        output.data<float>(),
